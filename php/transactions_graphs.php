...
function thai_month($month, $full = true){
...
$numDays = cal_days_in_month(CAL_GREGORIAN, $selected_month, $selected_year);
...
$sqlDaily = "SELECT DAY(stock_date) AS d, 
             ...
             FROM stock WHERE YEAR(stock_date)=? AND MONTH(stock_date)=? GROUP BY d";
...
$dailyImport = array_column($dailyData, 'import');
$dailyRemove = array_column($dailyData, 'remove');
...
$sqlMonthly = "SELECT MONTH(stock_date) AS m, 
                  SUM(CASE WHEN stock_type='import' THEN quantity ELSE 0 END) AS total_import,
                  SUM(CASE WHEN stock_type='remove' THEN quantity ELSE 0 END) AS total_remove
               FROM stock WHERE YEAR(stock_date)=? GROUP BY m";
...
$sqlYearly = "SELECT YEAR(stock_date) AS y, 
                 SUM(CASE WHEN stock_type='import' THEN quantity ELSE 0 END) AS total_import,
                 SUM(CASE WHEN stock_type='remove' THEN quantity ELSE 0 END) AS total_remove
              FROM stock GROUP BY y ORDER BY y ASC";
$resYearly = $conn->query($sqlYearly);
while($r = $resYearly->fetch_assoc()){
    $yearlyData[(int)$r['y']] = ['import' => (float)$r['total_import'], 'remove' => (float)$r['total_remove']];
}
$labelsYear = array_map(function($y){ return $y + 543; }, array_keys($yearlyData));
$yearlyImport = array_column($yearlyData, 'import');
$yearlyRemove = array_column($yearlyData, 'remove');
?>
<!DOCTYPE html>
<html lang="th">
<head>
    ...
    <style>
        :root { 
            --primary-color: #3498db; --secondary-color: #2980b9; --light-bg: #f5f8fa; 
...
<div class="sidebar" id="sidebar">
    <div class="sidebar-header">
        ...
        <h3>รายงานกราฟรายงานรายรับ-รายจ่าย</h3>
    </div>
    <a href="transactions_dashboard.php" class="<?= empty($type_filter) ? 'active' : '' ?>"><i class="fas fa-list fa-fw"></i>&nbsp; <span>รายการทั้งหมด</span></a>
    <a href="transactions_graphs.php" class="active"><i class="fas fa-chart-pie fa-fw"></i>&nbsp; <span>รายงานกราฟรายรับ-รายจ่าย</span></a>
</div>
...
<div class="main" id="main">
    ...
    <div class="container">
        <h2>รายวัน (<?= thai_month($selected_month, true) ?> <?= $selected_year + 543 ?>)</h2>
        <canvas id="dailyChart" height="120"></canvas>
    </div>
    
    <div class="container">
        <h2>รายเดือน (ปี <?= $selected_year + 543 ?>)</h2>
...
<div class="modal-overlay" id="emailModal">
    <div class="modal-content">
        ...
        <div class="modal-body">
            <p style="margin-bottom: 1rem;">รายงานกราฟสต็อกทั้งหมดจะถูกส่งเป็นไฟล์ PDF</p>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const chartOptions = { responsive: true, animation: false, plugins: { legend: { position: 'top', labels: { font: { family: 'Sarabun', size: 14 } } } }, scales: { y: { beginAtZero: true, ticks: { callback: value => new Intl.NumberFormat().format(value) } } } };
    
    const dailyCtx = document.getElementById('dailyChart');
    if(dailyCtx) dailyChart = new Chart(dailyCtx, { type: 'bar', data: { labels: <?= json_encode($labelsDay); ?>, datasets: [ { label:'รับเข้า', data: <?= json_encode($dailyImport) ?>, backgroundColor:'rgba(46, 204, 113, 0.8)', borderRadius:5 }, { label:'จ่ายออก', data: <?= json_encode($dailyRemove) ?>, backgroundColor:'rgba(231, 76, 60, 0.8)', borderRadius:5 } ] }, options: chartOptions });
    
    const monthlyCtx = document.getElementById('monthlyChart');
    if(monthlyCtx) monthlyChart = new Chart(monthlyCtx, { type: 'bar', data: { labels: <?= json_encode($labelsMonth); ?>, datasets: [ { label:'รับเข้า', data: <?= json_encode($monthlyImport) ?>, backgroundColor:'rgba(46, 204, 113, 0.8)', borderRadius:5 }, { label:'จ่ายออก', data: <?= json_encode($monthlyRemove) ?>, backgroundColor:'rgba(231, 76, 60, 0.8)', borderRadius:5 } ] }, options: chartOptions });
    
    const yearlyCtx = document.getElementById('yearlyChart');
    if(yearlyCtx) yearlyChart = new Chart(yearlyCtx, { type: 'line', data: { labels: <?= json_encode($labelsYear); ?>, datasets: [ { label:'รับเข้า', data: <?= json_encode($yearlyImport) ?>, borderColor:'#2ecc71', backgroundColor:'rgba(46, 204, 113, 0.2)', fill:true, tension:0.3 }, { label:'จ่ายออก', data: <?= json_encode($yearlyRemove) ?>, borderColor:'#e74c3c', backgroundColor:'rgba(231, 76, 60, 0.2)', fill:true, tension:0.3 } ] }, options: chartOptions });
    ...
    async function handleExport(action, recipientEmail = '') {
        ...
        if (action === 'download') {
            ...
            form.method = 'POST';
            form.action = 'export_stock_graphs.php'; // ใช้ไฟล์ export เดิม
            form.target = '_blank';
            ...
            try {
                const response = await fetch('send_stock_graphs_email.php', { method: 'POST', body: formData }); // ใช้ไฟล์ email เดิม
