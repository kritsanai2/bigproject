...
// --- เตรียม SQL Query ดึงข้อมูลที่คำนวณไว้แล้ว ---
$sql = "
    SELECT 
        e.employee_id,
        e.full_name,
...
while ($row = $result->fetch_assoc()) {
    $calculated_data[$row['employee_id']] = [
        ...
        'full'      => (int)($row['full_days'] ?? 0),
        'half'      => (int)($row['half_days'] ?? 0),
        'late'      => (int)($row['late_days'] ?? 0),
...
<style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .filter-box { display: flex; flex-wrap: wrap; justify-content: space-between; align-items: center; gap: 1.5rem; }
    .filter-form { display: flex; flex-wrap: wrap; gap: 1rem; align-items: center; }
    .filter-form label, .filter-form select { font-size: 1rem; }
    .filter-form select { padding: 10px; border: 1px solid var(--gray-border); border-radius: 8px; }
    .action-buttons { display: flex; gap: 0.75rem; }
    .action-button { padding: 10px 20px; border: none; border-radius: 8px; font-size: 1rem; font-weight: 600; cursor: pointer; color: white; display: inline-flex; align-items: center; gap: 8px; transition: all 0.2s; text-decoration: none; }
            ...
            <table>
                ...
                <tfoot>
                    <tr>
                        ...
                        <td><?= number_format($totals['leave'], 1) ?></td>
                        <td><?= number_format($totals['absent'], 1) ?></td>
...
<div class="modal-overlay" id="emailModal">
    <div class="modal-content">
        ...
        <div class="modal-body">
            ...
            <div id="emailStatus" style="margin-top: 1rem; text-align: center; min-height: 24px;"></div>
        </div>
        <div class="modal-footer">
            <button id="sendEmailButton" class="action-button">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (emailModal && emailModalButton) {
        ...
        const openModal = () => {
            emailModal.classList.add('active'); // Use class to show
        ...
        const closeModal = () => {
            emailModal.classList.remove('active'); // Use class to hide
        };
